...
onMounted( ()=>{ 
	loadQr() 
	//console.log('user',userInfo.value.isLogin )
...
<template>
	<!-- <div style="color: #999999;padding-bottom: 10px;text-align: center;">扫码登录</div> -->
<div style="text-align: center; position: relative;justify-content: center">

	 
	    <!-- <h1 class="text-2xl">微信登录</h1> -->
		<div v-if="qr.url"  class="wx-qr">
            
			<div class="z-10 w-[180px] h-[180px] absolute rounded-md  bg-white flex justify-center items-center" >
                <qrcode-vue :value="qr.url" :size="150" level="H" />
            </div>
			<div class=" z-10 w-[180px] h-[180px] absolute flex  cursor-pointer
			justify-center items-center  text-white bg-black/75 rounded-md flex-col  text-[20px]" @click="reflash()" v-if="st.timeout">
				<div class=" text-[50px]"><svgIcon icon="ic:baseline-refresh"></svgIcon></div>
				<div>{{ $t('ai.timeout4') }}</div>
				<div class="text-[16px]">{{ $t('ai.reflash') }}</div>
	...
	<!-- <div style="padding-top: 10px;"  >
      <el-divider> 
         <el-button type="primary" size="small"><a href="/oauth/github" style="color: #ffffff">{{ $t('ai.loginGithub') }}</a> </el-button>
      </el-divider>
     </div> -->
...
<style scoped>
.wx-qr{
	position: relative; justify-content: center; height: 200px;  -webkit-box-align: center; display: flex; margin: 10px 0;
